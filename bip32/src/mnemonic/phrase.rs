...
pub type Entropy = vec::Vec<u8>; //[u8; KEY_SIZE];
...
impl Phrase {
    ...
    pub fn random(mut rng: impl RngCore + CryptoRng, language: Language) -> Self {
        let mut entropy = vec![0_u8; KEY_SIZE / 2]; //Entropy::default();
    ...
    pub fn from_entropy(entropy: Entropy, language: Language) -> Self {
        ...
        let checksum_byte = Sha256::digest(&entropy).as_slice()[0];
        ...
        S: AsRef<str>,
    {
        ...
        let ent_len = entropy.len() - 1;
        if entropy.len() < KEY_SIZE / 2 + 1 || entropy.len() > KEY_SIZE + 1 || ent_len % 4 != 0 {
            return Err(Error::Bip39);
        }

        let actual_checksum = entropy[entropy.len() - 1];

        // Truncate to get rid of the byte containing the checksum
        entropy.truncate(ent_len);

        let expected_checksum = Sha256::digest(&entropy).as_slice()[0] & checksum_mask(ent_len);

        if actual_checksum != expected_checksum {
            return Err(Error::Bip39);
        }

        Ok(Self::from_entropy(entropy, language))
    }
